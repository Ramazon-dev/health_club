...
extension DialogExt on BuildContext {
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                subtitle,
                textAlign: TextAlign.center,
                style: TextStyle(fontWeight: FontWeight.w400, color: Colors.black, fontSize: 14.sp),
              ),
              SizedBox(height: 50.h),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  SecondaryButton(onPressed: onCancelled, text: cancelText ?? 'Отменить', size: Size(0.36.sw, 50.h)),
                  PrimaryButton(
                    size: Size(0.36.sw, 50.h),
                    onPressed: () {
                      context.router.maybePop();
  ...
  Future showDynamicDialog({required List<Widget> widgets}) {
    return showDialog(
      context: this,
      builder: (context) => Dialog(
